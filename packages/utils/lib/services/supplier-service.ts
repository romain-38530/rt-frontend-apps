...
const supplierApi = createApiClient({
  baseURL: process.env.NEXT_PUBLIC_SUPPLIER_API_URL || 'https://ddaywxps9n701.cloudfront.net/api/v1',
  timeout: 30000,
  retries: 3,
});
...
export class SupplierService {
  ...
  static async uploadOrderDocument(
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_SUPPLIER_API_URL || 'https://ddaywxps9n701.cloudfront.net/api/v1'}/suppliers/${supplierId}/orders/${orderId}/documents`,
      {
        method: 'POST',
        headers,
        body: formData,
