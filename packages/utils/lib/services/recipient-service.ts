...
const recipientApi = createApiClient({
  baseURL: process.env.NEXT_PUBLIC_RECIPIENT_API_URL || 'http://localhost:3021/api/v1',
  timeout: 30000,
  retries: 3,
});
...
export class RecipientService {
  ...
  static async uploadSignaturePhotos(
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_RECIPIENT_API_URL || 'http://localhost:3021/api/v1'}/recipients/${recipientId}/deliveries/${deliveryId}/photos`,
      {
        method: 'POST',
        headers,
        body: formData,
  ...
  static async uploadIncidentPhotos(
    ...
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_RECIPIENT_API_URL || 'http://localhost:3021/api/v1'}/recipients/${recipientId}/incidents/${incidentId}/photos`,
      {
        method: 'POST',
        headers,
        body: formData,
