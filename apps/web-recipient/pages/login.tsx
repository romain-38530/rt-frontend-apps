...
export default function Login() {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://ddaywxps9n701.cloudfront.net'}/auth/forgot-password`, {
        method: 'POST',
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://ddaywxps9n701.cloudfront.net'}/api/auth/login`, {
        method: 'POST',
        headers: {
      ...
      <div style={{
        ...
        <div style={{
          ...
          <form onSubmit={handleLogin}>
            ...
            <div style={{ textAlign: 'center', marginTop: '16px' }}>
              <button type="button" onClick={() => setShowForgotPassword(true)} style={{ background: 'none', border: 'none', color: '#11998e', fontSize: '14px', cursor: 'pointer', textDecoration: 'underline' }}>{t.forgotPassword}</button>
            </div>
          </form>
        ...
        {showForgotPassword && (
          <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
            <div style={{ background: 'white', borderRadius: '16px', padding: '40px', maxWidth: '400px', width: '90%', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
              ...
              {forgotSuccess ? (
                <div>
                  ...
                  <button onClick={() => { setShowForgotPassword(false); setForgotSuccess(false); setForgotEmail(''); }} style={{ width: '100%', padding: '12px', background: '#11998e', color: 'white', border: 'none', borderRadius: '6px', fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>{t.backToLogin}</button>
                </div>
              ) : (
                <form onSubmit={handleForgotPassword}>
                  ...
                  <button type="submit" disabled={forgotLoading} style={{ width: '100%', padding: '12px', background: forgotLoading ? '#a0aec0' : '#11998e', color: 'white', border: 'none', borderRadius: '6px', fontSize: '14px', fontWeight: '600', cursor: forgotLoading ? 'not-allowed' : 'pointer', marginBottom: '12px' }}>{forgotLoading ? t.sending : t.sendResetLink}</button>
