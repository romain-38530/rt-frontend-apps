...
import PreInvoice, { IPreInvoice, IPreInvoiceLine, ICarrierBankDetails, PreInvoiceStatus } from '../models/PreInvoice';
...
class PreInvoiceService {
   ...
   */
  private static generatePreInvoiceNumber(year: number, month: number): string {
   ...
   */
  static async analyzeCMR(orderId: string): Promise<CMRAnalysis> {
    const analysis: CMRAnalysis = {
      waitingHours: 0,
      delayHours: 0,
      notes: []
    };
    ...
    try {
      ...
      const cmrDoc = await Document.findOne({
        orderId,
        type: 'cmr',
        status: 'validated'
      });
   ...
   */
  static async addCompletedOrder(orderId: string): Promise<IPreInvoice | null> {
    try {
      const order = await Order.findOne({ orderId });
      if (!order) {
      ...
      let preInvoice = await PreInvoice.findOne({
      ...
      if (!preInvoice) {
        preInvoice = new PreInvoice({
          preInvoiceId: `preinv_${uuidv4()}`,
          preInvoiceNumber: this.generatePreInvoiceNumber(year, month),
          period: { month, year, startDate, endDate },
          industrialId: order.industrialId,
          industrialName: (order as any).industrialName || order.industrialId,
      ...
      const requiredDocs = await Document.countDocuments({
        orderId,
        type: { $in: ['cmr', 'bl', 'pod'] },
        status: 'validated'
      });
   ...
   */
  private static recalculateTotals(preInvoice: IPreInvoice): void {
    const totals = {
      baseAmount: 0,
      waitingAmount: 0,
      delayPenalty: 0,
    ...
    for (const line of preInvoice.lines) {
      ...
      if (line.kpiData.onTimePickup) onTimePickupCount++;
      if (line.kpiData.onTimeDelivery) onTimeDeliveryCount++;
      if (line.kpiData.documentsComplete) documentsCompleteCount++;
      if (line.kpiData.incidentFree) incidentFreeCount++;
    }
   ...
   */
  static async sendMonthlyPreInvoicesToIndustrials(): Promise<number> {
    try {
      ...
      for (const preInvoice of preInvoices) {
        try {
          ...
          await NotificationService.sendPreInvoiceValidationRequest(
   ...
   */
  static async validateByIndustrial(
    preInvoiceId: string,
    validatedBy: string,
    comments?: string,
    ...
    try {
      const preInvoice = await PreInvoice.findOne({ preInvoiceId });
      ...
      await NotificationService.notifyCarrierPreInvoiceValidated(
        preInvoice.carrierEmail,
        preInvoice.carrierName,
        preInvoice.preInvoiceNumber,
        preInvoice.totals.totalTTC
      );
   ...
   */
  static async uploadCarrierInvoice(
    preInvoiceId: string,
    invoiceData: {
   ...
   */
  static async performInvoiceControl(preInvoiceId: string): Promise<IPreInvoice | null> {
    try {
      const preInvoice = await PreInvoice.findOne({ preInvoiceId });
      if (!preInvoice || !preInvoice.carrierInvoice) return null;
      ...
      if (autoAccepted) {
        ...
        // Notifier le transporteur
        await NotificationService.notifyCarrierInvoiceAccepted(
          preInvoice.carrierEmail,
          preInvoice.carrierName,
          preInvoice.preInvoiceNumber,
          preInvoice.carrierInvoice.invoiceAmount,
          dueDate
        );
        ...
        await NotificationService.notifyCarrierInvoiceRejected(
          preInvoice.carrierEmail,
          preInvoice.carrierName,
          preInvoice.preInvoiceNumber,
          preInvoiceAmount,
   ...
   */
  static async updatePaymentCountdowns(): Promise<number> {
    try {
      const preInvoices = await PreInvoice.find({
      ...
      for (const preInvoice of preInvoices) {
        if (preInvoice.payment?.dueDate) {
          ...
          if ((previousDays === 6 && daysRemaining <= 5) || (previousDays === 3 && daysRemaining <= 2)) {
            try {
              await NotificationService.sendPaymentReminderToIndustrial(
                preInvoice.industrialEmail,
                preInvoice.industrialName,
                preInvoice.preInvoiceNumber,
                preInvoice.carrierName,
                preInvoice.carrierInvoice?.invoiceAmount || preInvoice.totals.totalTTC,
                daysRemaining,
                dueDate
              );
          ...
          if (daysRemaining < 0) {
            ...
            if (daysOverdue === 1 || daysOverdue === 3 || daysOverdue === 7 || daysOverdue % 7 === 0) {
              try {
                await NotificationService.sendOverduePaymentAlert(
                  preInvoice.industrialEmail,
                  preInvoice.industrialName,
                  preInvoice.preInvoiceNumber,
                  preInvoice.carrierName,
  ...
  static async markAsPaid(
    ...
    try {
      ...
      // Notifier le transporteur
      await NotificationService.notifyCarrierPaymentSent(
        preInvoice.carrierEmail,
        preInvoice.carrierName,
        preInvoice.preInvoiceNumber,
        paidAmount,
   ...
   */
  static async generatePaymentExport(): Promise<any[]> {
    try {
      const preInvoices = await PreInvoice.find({
        status: 'payment_pending',
  ...
  static async getPreInvoices(filters: {
    ...
    if (filters.industrialId) query.industrialId = filters.industrialId;
    ...
    return PreInvoice.find(query).sort({ createdAt: -1 });
  }

  /**
   * Récupère une préfacture par ID
   */
  static async getPreInvoiceById(preInvoiceId: string): Promise<IPreInvoice | null> {
    return PreInvoice.findOne({ preInvoiceId });
  }

  /**
   * Récupère une préfacture par numéro
   */
  static async getPreInvoiceByNumber(preInvoiceNumber: string): Promise<IPreInvoice | null> {
    return PreInvoice.findOne({ preInvoiceNumber });
  }
}
