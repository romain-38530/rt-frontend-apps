...
interface DeliveryConfirmationParams {
  ...
  signature: {
    data: string;  // Base64 de la signature
    timestamp: Date;
    ipAddress?: string;
    deviceInfo?: string;
  };
...
class DeliveryService {
   ...
   */
  static async confirmDelivery(params: DeliveryConfirmationParams): Promise<{
    success: boolean;
    order?: any;
    error?: string;
  }> {
    try {
      ...
      (order as any).deliveryConfirmation = {
   ...
   */
  static async reportDeliveryIssue(params: DeliveryIssueParams): Promise<{
    success: boolean;
    issueId?: string;
    error?: string;
  }> {
    try {
      ...
      if (!(order as any).deliveryIssues) {
        (order as any).deliveryIssues = [];
      }

      (order as any).deliveryIssues.push({
        issueId,
      ...
      await EventService.createEvent({
        ...
        source: params.reportedBy.role as any,
        data: {
          issueId,
          issueType: params.issueType,
          severity: params.severity,
   ...
   */
  private static async triggerPostDeliveryActions(order: any): Promise<void> {
    try {
      // Vérifier les documents
      const docCheck = await DocumentService.checkRequiredDocuments(order.orderId);

      if (docCheck.complete && order.carrierId) {
        // Calculer le score
        await ScoringService.calculateScore(order.orderId, order.carrierId);
   ...
   */
  private static async notifyDeliveryConfirmed(order: any, params: DeliveryConfirmationParams): Promise<void> {
   ...
   */
  private static async notifyDeliveryIssue(order: any, params: DeliveryIssueParams, issueId: string): Promise<void> {
   ...
   */
  static async getDeliveryStats(industrialId?: string): Promise<{
    ...
    const matchStage: any = { status: { $in: ['delivered', 'completed', 'archived'] } };
    if (industrialId) {
    ...
    for (const order of orders) {
      ...
      if ((order as any).deliveryIssues?.length > 0) {
        withIssues++;
      }
    }
