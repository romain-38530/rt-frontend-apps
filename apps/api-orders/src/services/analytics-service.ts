...
import { CarrierGlobalScore, CarrierOrderScore } from '../models/CarrierScore';
...
export interface DashboardSummary {
  ...
  // Top performers
  topCarriers: Array<{ carrierId: string; carrierName: string; score: number; ordersCount: number }>;
...
export class AnalyticsService {
  ...
  static async getCarrierKPIs(
    ...
    const dispatchChains = await DispatchChain.find({
      industrialId,
    ...
    const carrierScore = await CarrierGlobalScore.findOne({ carrierId }).lean();
        ...
        if (attempt.carrierId === carrierId) {
          ...
          if (attempt.status === 'accepted') {
            ...
            if (attempt.respondedAt && attempt.sentAt) {
              responseTimes.push(
                (new Date(attempt.respondedAt).getTime() - new Date(attempt.sentAt).getTime()) / 60000
              );
            }
    ...
    for (const order of orders) {
      if (order.status === 'delivered' || order.status === 'closed' || order.status === 'completed') {
        ...
        if (order.dates?.actualPickupDate && order.dates?.pickupDate) {
          ...
          if (delayMinutes <= 15) onTimePickup++;
          else if (delayMinutes > 0) pickupDelays.push(delayMinutes);
        } else {
        ...
        if (order.dates?.actualDeliveryDate && order.dates?.deliveryDate) {
          ...
          if (delayMinutes <= 30) onTimeDelivery++;
          else if (delayMinutes > 0) deliveryDelays.push(delayMinutes);
        } else {
          onTimeDelivery++;
        }
      }
    ...
    const completedOrders = orders.filter(o =>
      o.status === 'delivered' || o.status === 'closed' || o.status === 'completed'
    );
   ...
   */
  static async getIndustrialKPIs(
    industrialId: string,
    carrierId: string,
    startDate: Date,
    endDate: Date
  ): Promise<IndustrialKPIs> {
    ...
    const allCarrierOrders = await Order.find({
      carrierId,
      createdAt: { $gte: startDate, $lte: endDate }
    }).lean();
    ...
    let frequency: 'regular' | 'occasional' | 'sporadic' = 'sporadic';
    if (ordersPerWeek >= 3) frequency = 'regular';
    else if (ordersPerWeek >= 1) frequency = 'occasional';
   ...
   */
  static async getLogisticianKPIs(
    userId: string,
    startDate: Date,
    endDate: Date
  ): Promise<LogisticianKPIs> {
    ...
    for (const chain of dispatchChains) {
      if (chain.status === 'completed') {
        ...
        if (acceptedIndex === 0) firstAttemptSuccess++;
        if (acceptedIndex >= 0) attemptCounts.push(acceptedIndex + 1);
      }
    }
   ...
   */
  static async getIndustrialDashboard(
    industrialId: string,
    startDate: Date,
    endDate: Date
    ...
    const inProgressOrders = orders.filter(o =>
      ['in_transit', 'arrived_pickup', 'loaded', 'arrived_delivery', 'carrier_accepted'].includes(o.status)
    );
    const cancelledOrders = orders.filter(o => o.status === 'cancelled');

    // Aggreger par transporteur
    const carrierStats: Record<string, { orders: number; score: number; name: string }> = {};
    ...
    if (carrierIds.length > 0) {
      const scores = await CarrierGlobalScore.find({ carrierId: { $in: carrierIds } }).lean();
    ...
    const pendingAssignments = orders.filter(o =>
      o.status === 'created' || o.status === 'sent_to_carrier'
    ).length;
   ...
   */
  static async getCarriersRanking(
    industrialId: string,
    startDate: Date,
    endDate: Date
  ): Promise<Array<CarrierKPIs & { ranking: number }>> {
    ...
    const carrierIds = [...new Set(orders.map(o => o.carrierId).filter(Boolean))] as string[];

    // Calculer KPIs pour chaque transporteur
    const kpisList: CarrierKPIs[] = [];
   ...
   */
  static async getIndustrialsRanking(
    carrierId: string,
    startDate: Date,
    endDate: Date
  ): Promise<IndustrialKPIs[]> {
