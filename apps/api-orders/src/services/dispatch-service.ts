...
import Lane, { ILane, ILaneCarrier } from '../models/Lane';
import DispatchChain, { IDispatchChain, IDispatchAttempt } from '../models/DispatchChain';
import { CarrierGlobalScore } from '../models/CarrierScore';
import Order from '../models/Order';
import EventService from './event-service';
import PortalInvitationService from './portal-invitation-service';
import NotificationService from './notification-service';
import AffretiaService from './affretia-service';

// Interface pour les infos transporteur (à récupérer depuis un service externe)
...
class DispatchService {
   ...
   */
  static async detectLane(orderId: string): Promise<ILane | null> {
    ...
    if (matchedLanes.length === 0) {
      ...
      await EventService.escalatedToAffretia(
        orderId,
        order.reference,
        'Aucune ligne de transport configurée pour ce trajet'
      );
      return null;
    }
    ...
    await EventService.laneDetected(
      orderId,
      order.reference,
      bestMatch.lane.laneId,
      bestMatch.lane.name
    );
   ...
   */
  private static async findMatchingLanes(order: any): Promise<MatchedLane[]> {
    const lanes = await Lane.find({ isActive: true });
    const matches: MatchedLane[] = [];
   ...
   */
  private static evaluateLaneMatch(lane: ILane, order: any): { isMatch: boolean; score: number; criteria: string[] } {
    let score = 0;
    ...
    if (order.constraints && order.constraints.length > 0 && lane.requiredConstraints) {
      const orderConstraints = order.constraints.map((c: any) => c.type);
      const matchedConstraints = orderConstraints.filter((c: string) =>
        lane.requiredConstraints!.includes(c)
      );
  ...
  private static checkLocationMatch(
    laneLoc: ILane['origin'],
    address: any
  ): { matches: boolean; score: number; matchType: string } {
    if (!address) return { matches: false, score: 0, matchType: '' };
   ...
   */
  static async generateDispatchChain(orderId: string, lane: ILane): Promise<IDispatchChain> {
    const order = await Order.findOne({ orderId });
    if (!order) throw new Error('Commande non trouvée');
    ...
    const activeCarriers = lane.carriers.filter((c: ILaneCarrier) => c.isActive);
    ...
    const attempts: IDispatchAttempt[] = carriersWithCombinedScore
      .map((item, index) => ({
    ...
    // Enregistrer l'événement
    await EventService.dispatchChainGenerated(
      orderId,
      order.reference,
      chainId,
      attempts.length
   ...
   */
  static async startDispatch(chainId: string): Promise<IDispatchAttempt> {
    const chain = await DispatchChain.findOne({ chainId });
    if (!chain) throw new Error('Chaîne de dispatch non trouvée');
   ...
   */
  static async sendToNextCarrier(chain: IDispatchChain): Promise<IDispatchAttempt> {
    ...
    // Enregistrer l'événement
    await EventService.orderSentToCarrier(
      chain.orderId,
   ...
   */
  private static async sendCarrierNotification(
    chain: IDispatchChain,
    attempt: IDispatchAttempt,
    expiresAt: Date
  ): Promise<void> {
    try {
      ...
      const sent = await NotificationService.sendCarrierInvitation({
   ...
   */
  private static async getCarrierInfo(carrierId: string, laneId: string): Promise<CarrierInfo | null> {
    try {
   ...
   */
  static async handleCarrierAccept(
    chainId: string,
    carrierId: string,
    proposedPrice?: number
  ): Promise<IDispatchChain> {
    const chain = await DispatchChain.findOne({ chainId });
    if (!chain) throw new Error('Chaîne de dispatch non trouvée');

    const attempt = chain.attempts.find((a: IDispatchAttempt) =>
      a.carrierId === carrierId && a.status === 'sent'
    );
    if (!attempt) throw new Error('Tentative non trouvée ou déjà traitée');
    ...
    if (updatedOrder) {
      ...
      if (invitationIds.length > 0) {
        await Order.findOneAndUpdate(
          { orderId: chain.orderId },
          { $push: { portalInvitations: { $each: invitationIds } } }
        );
      }
      ...
      if (carrierInfo?.email) {
        ...
        await NotificationService.sendCarrierConfirmation(
          ...
          attempt.carrierName,
          chain.orderReference,
          `${portalUrl}/orders/${chain.orderId}`
        );
   ...
   */
  private static async notifyIndustrialStatus(
    chain: IDispatchChain,
    ...
    try {
      ...
      const industrialName = (order as any).industrialName || chain.industrialId;

      await NotificationService.notifyIndustrialDispatchStatus(
        industrialEmail,
        industrialName,
        chain.orderReference,
        status,
   ...
   */
  static async handleCarrierRefuse(
    chainId: string,
    carrierId: string,
    reason?: string
  ): Promise<IDispatchChain> {
    ...
    // Enregistrer l'événement
    await EventService.carrierRefused(
      chain.orderId,
      chain.orderReference,
      carrierId,
      attempt.carrierName,
   ...
   */
  static async handleCarrierTimeout(chainId: string, attemptIndex: number): Promise<IDispatchChain> {
    const chain = await DispatchChain.findOne({ chainId });
    if (!chain) throw new Error('Chaîne de dispatch non trouvée');
    ...
    // Enregistrer l'événement
    await EventService.carrierTimeout(
      chain.orderId,
      chain.orderReference,
      attempt.carrierId,
      attempt.carrierName
   ...
   */
  static async escalateToAffretia(chain: IDispatchChain, reason: string): Promise<void> {
    ...
    try {
      const response = await AffretiaService.escalateOrder(chain);
   ...
   */
  static async getDispatchStatus(orderId: string): Promise<IDispatchChain | null> {
    return DispatchChain.findOne({ orderId }).sort({ createdAt: -1 });
  }

  /**
   * Récupère toutes les lanes actives
   */
  static async getActiveLanes(): Promise<ILane[]> {
    return Lane.find({ isActive: true }).sort({ name: 1 });
  }

  /**
   * Crée une nouvelle lane
   */
  static async createLane(laneData: Partial<ILane>): Promise<ILane> {
    const lane = new Lane({
      laneId: `lane_${uuidv4()}`,
      ...laneData
    });
   ...
   * Met à jour une lane
   */
  static async updateLane(laneId: string, updates: Partial<ILane>): Promise<ILane | null> {
    return Lane.findOneAndUpdate(
      { laneId },
      { $set: updates },
      { new: true }
    );
