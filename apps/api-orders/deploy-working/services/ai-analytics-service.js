...
class AIAnalyticsService {
     ...
     */
    static async callClaudeAPI(systemPrompt, userPrompt) {
     ...
     */
    static generateRecommendations(kpis, type) {
        const recommendations = [];
        if (type === 'carrier') {
            const carrierKpis = kpis;
     ...
     */
    static generateTrends(currentKpis, previousKpis) {
        const trends = [];
     ...
     */
    static generateAlerts(kpis, type) {
        const alerts = [];
     ...
     */
    static async generateIndustrialReport(industrialId, industrialName, month, year) {
        const startTime = Date.now();
        const reportId = `RPT-${year}${String(month).padStart(2, '0')}-IND-${(0, uuid_1.v4)().slice(0, 8)}`;
        // Calculer les dates de la période
        const startDate = new Date(year, month - 1, 1);
        const endDate = new Date(year, month, 0, 23, 59, 59);
        // Récupérer le classement des transporteurs
        const carriersRanking = await analytics_service_1.default.getCarriersRanking(industrialId, startDate, endDate);
        // Générer les analyses par transporteur
        const carrierAnalyses = carriersRanking.map(carrier => {
            const recommendations = this.generateRecommendations(carrier, 'carrier');
            const strengths = [];
            const weaknesses = [];
            if (carrier.operational.acceptanceRate >= 90)
                strengths.push('Excellent taux d\'acceptation');
            if (carrier.punctuality.onTimeDeliveryRate >= 95)
                strengths.push('Ponctualité exemplaire');
            if (carrier.globalScore >= 85)
                strengths.push('Performance globale remarquable');
            if (carrier.operational.acceptanceRate < 70)
                weaknesses.push('Taux d\'acceptation insuffisant');
            if (carrier.punctuality.onTimeDeliveryRate < 85)
                weaknesses.push('Retards fréquents');
            if (carrier.incidents.incidentRate > 5)
                weaknesses.push('Taux d\'incidents élevé');
            return {
        ...
        // Créer le rapport
        const report = new AIReport_1.default({
            reportId,
     ...
     */
    static async generateCarrierReport(carrierId, carrierName, month, year) {
        const startTime = Date.now();
        const reportId = `RPT-${year}${String(month).padStart(2, '0')}-CAR-${(0, uuid_1.v4)().slice(0, 8)}`;
        const startDate = new Date(year, month - 1, 1);
        const endDate = new Date(year, month, 0, 23, 59, 59);
        // Récupérer le classement des industriels
        const industrialsRanking = await analytics_service_1.default.getIndustrialsRanking(carrierId, startDate, endDate);
        // Générer les analyses par industriel
        const industrialAnalyses = industrialsRanking.map(ind => {
            const recommendations = this.generateRecommendations(ind, 'industrial');
            const strengths = [];
            const concerns = [];
            if (ind.activity.revenueShare >= 20)
                strengths.push('Client stratégique (>20% CA)');
            if (ind.payments.onTimePaymentRate >= 90)
                strengths.push('Payeur fiable');
            if (ind.relationship.orderFrequency === 'regular')
                strengths.push('Activité régulière');
            if (ind.workingConditions.averageWaitingTimeMinutes > 90)
                concerns.push('Temps d\'attente élevés');
            if (ind.payments.averagePaymentDelayDays > 45)
                concerns.push('Délais de paiement longs');
            // Calculer un score de relation
            let relationScore = 50;
            if (ind.payments.onTimePaymentRate >= 90)
                relationScore += 20;
            if (ind.workingConditions.averageWaitingTimeMinutes <= 60)
                relationScore += 15;
            if (ind.relationship.orderFrequency === 'regular')
                relationScore += 15;
            return {
        ...
        const alerts = highDependency.map(i => ({
            severity: 'warning',
     ...
     */
    static async generateLogisticianReport(userId, userName, month, year) {
        const startTime = Date.now();
        const reportId = `RPT-${year}${String(month).padStart(2, '0')}-LOG-${(0, uuid_1.v4)().slice(0, 8)}`;
        const startDate = new Date(year, month - 1, 1);
        const endDate = new Date(year, month, 0, 23, 59, 59);
        ...
        if (kpis.assignments.firstAttemptSuccessRate >= 80)
            strengths.push('Excellent ciblage des transporteurs');
        if (kpis.performance.serviceRate >= 95)
            strengths.push('Taux de service remarquable');
        if (kpis.productivity.pendingOrders <= 5)
            strengths.push('Bonne gestion du backlog');
        if (kpis.assignments.firstAttemptSuccessRate < 60)
            areasForImprovement.push('Améliorer le ciblage transporteurs');
        if (kpis.productivity.pendingOrders > 10)
            areasForImprovement.push('Réduire le backlog');
        if (kpis.assignments.escalationToAffretiaRate > 20)
            areasForImprovement.push('Réduire les escalades');
        const overallScore = (kpis.performance.serviceRate + kpis.assignments.firstAttemptSuccessRate) / 2;
        const logisticianAnalysis = {
            userId,
            userName,
     ...
     */
    static async getLatestReport(entityType, entityId) {
        return AIReport_1.default.findOne({
     ...
     */
    static async listReports(entityType, entityId, limit = 12) {
        return AIReport_1.default.find({
            'targetEntity.type': entityType,
            'targetEntity.id': entityId,
            status: 'completed'
        })
     ...
     */
    static async submitFeedback(reportId, rating, helpful, comment) {
        return AIReport_1.default.findOneAndUpdate({ reportId }, {
            userFeedback: {
                rating,
                helpful,
                comment,
...
exports.default = AIAnalyticsService;
