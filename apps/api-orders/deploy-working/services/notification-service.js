...
const transporter = isSmtpConfigured
    ? nodemailer_1.default.createTransport(smtpConfig)
    : null;
...
class NotificationService {
     ...
     */
    static async sendCarrierInvitation(params) {
        const { carrierName, carrierEmail, orderReference, pickupCity, deliveryCity, pickupDate, deliveryDate, goodsDescription, weight, expiresAt, responseUrl } = params;
     ...
     */
    static async sendEmail(mailOptions, recipient) {
        if (!transporter) {
     ...
     */
    static async sendTimeoutReminder(params) {
     ...
     */
    static async sendCarrierConfirmation(carrierEmail, carrierName, orderReference, portalUrl) {
        const html = `
      <!DOCTYPE html>
      <html>
      <head>
     ...
     */
    static async notifyIndustrialDispatchStatus(industrialEmail, industrialName, orderReference, status, carrierName) {
     ...
     */
    static async notifyIndustrialInvoiceUploaded(industrialEmail, industrialName, preInvoiceNumber, carrierName, invoiceNumber, invoiceAmount, preInvoiceAmount) {
