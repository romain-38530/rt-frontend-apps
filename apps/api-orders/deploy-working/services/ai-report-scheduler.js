...
const ai_analytics_service_1 = __importDefault(require("./ai-analytics-service"));
...
const transporter = isSmtpConfigured ? nodemailer_1.default.createTransport(smtpConfig) : null;
class AIReportScheduler {
     ...
     */
    async generateReportForEntity(entity, month, year) {
        console.log(`[AI Report Scheduler] Generating ${entity.type} report for ${entity.name}`);
        let report;
        switch (entity.type) {
            case 'industrial':
                report = await ai_analytics_service_1.default.generateIndustrialReport(entity.id, entity.name, month, year);
                break;
            case 'carrier':
                report = await ai_analytics_service_1.default.generateCarrierReport(entity.id, entity.name, month, year);
                break;
            case 'logistician':
                report = await ai_analytics_service_1.default.generateLogisticianReport(entity.id, entity.name, month, year);
                break;
        }
     ...
     */
    async getActiveEntities(month, year) {
        const entities = [];
        const startDate = new Date(year, month - 1, 1);
        const endDate = new Date(year, month, 0, 23, 59, 59);
        // Industriels actifs (ont eu des commandes)
        const industrials = await Order_1.default.aggregate([
            {
                $match: {
        ...
        const carriers = await Order_1.default.aggregate([
            {
                $match: {
        ...
        // Logisticiens actifs
        const logisticians = await Order_1.default.aggregate([
            {
                $match: {
     ...
     */
    async sendReportNotification(entity, report) {
...
L'équipe SYMPHONI.A
     ...
     */
    async generateManualReport(entityType, entityId, entityName, month, year) {
        const now = new Date();
        ...
        switch (entityType) {
            case 'industrial':
                return ai_analytics_service_1.default.generateIndustrialReport(entityId, entityName, targetMonth, targetYear);
            case 'carrier':
                return ai_analytics_service_1.default.generateCarrierReport(entityId, entityName, targetMonth, targetYear);
            case 'logistician':
                return ai_analytics_service_1.default.generateLogisticianReport(entityId, entityName, targetMonth, targetYear);
        }
    }
}
exports.default = new AIReportScheduler();
//# sourceMappingURL=ai-report-scheduler.js.map